...
/**
 * Saqlash joyi:
 *  - BLOB_READ_WRITE_TOKEN bo'lsa → Vercel Blob (doimiy; Vercel'da disk vaqtinchalik!)
 *  - bo'lmasa → public/uploads (lokal dev)
 */
async function saveFile(name: string, data: Buffer, contentType: string) {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    const { put } = await import("@vercel/blob");
    const blob = await put(`products/${name}`, data, {
      access: "public",
      contentType,
...
export async function POST(req: Request) {
  ...
  if (file.type === "image/svg+xml") {
    const url = await saveFile(`${stamp}.svg`, input, "image/svg+xml");
  ...
  try {
    ...
    const url = await saveFile(`${stamp}.webp`, output, "image/webp");
