...
const METALS = {
  ...
  steel: ["#8fc3dc", "#5b96b4", "#356c88", "#bfe0ef"],
...
function studio(theme = "light") {
  ...
  const bgTop = dark ? "#0d2e3d" : "#fdfeff";
  const bgBot = dark ? "#04222f" : "#e7eff4";
  const floor = dark ? "#062733" : "#dbe6ec";
  return {
    defs: `
    ...
    <radialGradient id="spot" cx="0.5" cy="0.28" r="0.75">
      <stop offset="0%" stop-color="${dark ? "#2e7ea0" : "#ffffff"}" stop-opacity="${dark ? 0.16 : 0.95}"/>
...
function sceneBollard(theme = "light") {
  const s = studio(theme);
  const m = uid("m");
  const defs = `${s.defs}${defsMetal(m, METALS.steel, 0)}`;
  const body = `${s.bg}
...
const FILES = {
  ...
  "rolstavni-steel": () => sceneRollerShutter(METALS.steel, "light"),
  ...
  "panjara-steel": () => sceneGrille(METALS.steel, "light"),
